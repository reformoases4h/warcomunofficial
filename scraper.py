...
def main():
    ...
    if is_cold_start:
        ...
        # is sometimes wrong -- but far more often it's simply accurate for
        # genuinely old articles). Show the best-guess top MAX_ARTICLES by
        # lastmod as the initial batch, then lock EVERY sitemap URL in as
        # "already seen" -- including the ones not selected for display.
        #
        # This deliberately accepts a narrow, one-time risk: if a brand-new
        # article happens to both exist at this exact moment AND carry a
        # badly wrong (old-looking) lastmod, it could be excluded from the
        # initial batch and never resurface. That's a rare coincidence.
        # The alternative (leaving unselected articles "unclassified" so
        # they could still surface later) was tried and caused a much more
        # common, ongoing problem instead: dozens of genuinely old articles
        # that simply didn't make the initial cut trickled in as false
        # "new" detections one at a time over the following days, since
        # nothing distinguished "old article that existed all along" from
        # "genuinely new since last run" once they were left unclassified.
        dated = [(url, lm) for url, lm in sitemap.items() if lm]
        dated.sort(key=lambda kv: kv[1], reverse=True)
        display_urls = {url for url, _ in dated[:MAX_ARTICLES]}

        for url, lastmod_iso in sitemap.items():
            if url in display_urls:
